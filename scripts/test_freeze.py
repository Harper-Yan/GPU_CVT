...
def lloyd_iter(objname, S, P0, F0, N0, vf, treeP0, it, frozen, prev_idxn, freeze_disp=1e-4, k_neigh=20, k_proj=5):
    ...
    if frozen is not None:
        ...
        if prev_idxn is None:
            stable_nbr = np.zeros(len(S), dtype=bool)
        else:
            stable_nbr = np.all(np.sort(idxn, axis=1) == np.sort(prev_idxn, axis=1), axis=1)

        cand = (disp < float(freeze_disp)) & stable_nbr
        print(f"  same neighbors: {np.count_nonzero(stable_nbr)}")
        print(f"  low displacement: {np.count_nonzero(disp < freeze_disp)}")
    ...
    print("\nTangent frames (Unew,Vnew)[0..9]:")
    for i in range(10):
        print(i, "U", Unew[i], "V", Vnew[i])

    print("\nSite neighbors idxn2[0..9] (first 10 each):")
    for i in range(10):
        print(i, idxn2[i][:10])

    print("\nR2 =", R2)

    print("\nVoronoi polygons polys2d2[0..9]:")
    for i in range(10):
        p = polys2d2[i]
        print(i, "nverts =", 0 if p is None else len(p))
        if p is not None:
            for k in range(min(10, len(p))):
                print("   ", p[k])

    print("\nExtracted faces touching sites 0..9:")
    for f in Fnew:
        if f[0] < 10 or f[1] < 10 or f[2] < 10:
            print(f)


    out_path = f"freeze/{objname}/iter_{it:03d}.obj"
    out_dir = os.path.dirname(out_path)

    # create directory tree if it does not exist
    os.makedirs(out_dir, exist_ok=True)

    # write the obj using your function
    write_obj(out_path, Snew, Fnew)

    print("iter", it, "V", Snew.shape[0], "F", Fnew.shape[0], "frozen", int(np.sum(frozen)) if frozen is not None else 0, "wrote", out_path)
    return Snew, frozen, idxn
...
def main():
    ...
    S = P0.copy()
    frozen = np.zeros(len(S), dtype=bool)
    prev_idxn = None
    for it in range(iters):
        S, frozen, prev_idxn = lloyd_iter(objname, S, P0, F0, N0, vf, treeP0, it, frozen, prev_idxn, freeze_disp=freeze_disp, k_neigh=k_neigh, k_proj=k_proj)
        break
