...
**Recent GPU CVT accelerations focus on improving per-iteration accuracy or using approximation shortcuts, rather than reducing redundant computation.** Yao et al. [2023] introduced restricted tangent faces (RTF) and power-diagram variants (PowerRTF) computed directly on GPU without auxiliary points or full Voronoi traversal, evaluated at up to 30K sites. Fei et al. [2025] proposed curvature-adaptive multiple-facet clipping, where each Voronoi cell is clipped against 1 to 3 original mesh facets depending on local curvature (determined by the cosine angle between neighboring facet normals). Their method improves Voronoi cell accuracy at high curvature compared to single-tangent-plane methods, achieving higher element quality than PowerRTF (e.g., Q_avg 0.917 vs 0.869 on the Duck model) at the cost of slower per-iteration speed. They evaluate on meshes up to 100K input vertices with 3K to 9K output sites. In 2025, CGAL added an approximated discrete CVT remesher (GSoC project) that uses entirely discrete geometry processing and scales to several million triangles with low memory overhead.

These approaches address a complementary bottleneck to ours: they improve the *accuracy* or *approximation quality* of each Lloyd iteration, while we reduce the *number of sites* that require computation per iteration. The two strategies are orthogonal — a curvature-adaptive clipping scheme could be combined with our freeze policy, using multi-facet clipping for active sites while skipping KNN entirely for frozen ones.

A consistent pattern across all prior GPU CVT work is that **each Lloyd iteration treats all sites uniformly**, performing KNN/centroid/projection or equivalent for every site regardless of convergence state. Our method is, to our knowledge, the first to exploit the spatially non-uniform convergence behavior of Lloyd iteration to selectively reduce per-iteration workload. [...]
