...
**Streak vs. single-shot freeze test.** A single-shot test — freezing a site the first time both gates pass — produces a 64\% false-freeze rate at curved regions, because transient pauses in oscillation mimic convergence. Requiring a streak of $\geq$10 consecutive passes reduces the false-freeze rate to below 2\%, with the exact threshold scaled by curvature tier to balance safety against early freeze opportunity.
